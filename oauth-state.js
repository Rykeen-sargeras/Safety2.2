...
const app=express();
const VERIFICATION_CHANNEL_ID='1535094192244523128';
...
<div class="card">
  <h1>YouTube Membership Verification</h1>
  ...
  <div class="row">
    ...
    <a class="btn gray" href="/verify?mode=recheck">Recheck Membership</a>
  </div>
</div>

<div class="card">
  <h2>Creator Connect</h2>
  <p>If you are a creator being added to this server, click below and enter the creator password.</p>
  <a class="btn green" href="/creator-connect">Creator Connect</a>
</div>`)));
...
app.get('/creator-connect',(_req,res)=>{
  res.send(page('Creator Connect',`
  <div class="card" style="max-width:500px;margin:80px auto">
    <h1>Creator Connect</h1>
    <p>Enter the creator password to connect your YouTube channel.</p>
    <form method="post" action="/creator-connect">
      <p><input style="width:95%" type="password" name="password" placeholder="Creator password" required></p>
      <button class="btn green" type="submit">Continue to YouTube</button>
    </form>
  </div>`));
});
  ...
  try{
    if(!passwordMatches(req.body.password)){
      ...
      <div class="card" style="max-width:500px;margin:80px auto">
        ...
        <p>The creator password was not accepted.</p>
        <a class="btn" href="/creator-connect">Try Again</a>
      </div>`));
    }

    const state=await createState('google_creator_password','creator-connect');
...
app.get('/admin',async(req,res,next)=>{
  try{
    ...
    <div class="card"><h2>Creator Invites</h2>
    <p>Create a single-use link and send it to a creator. They can connect their YouTube channel without seeing your admin dashboard.</p>
    <form method="post" action="/admin/creator-invite" class="row">
      <label>Expires in</label>
      <select name="hours"><option value="1">1 hour</option><option value="6">6 hours</option><option value="24" selected>24 hours</option><option value="72">3 days</option></select>
      <button class="btn" type="submit">Create Invite Link</button>
    </form></div>
    <div class="card"><h2>Creator Connect</h2>
    <a class="btn" href="/admin/creator/connect">Connect YouTube Creator</a></div>
    <div class="card"><h2>Post Verification Panel</h2>
    <p>Default verification channel: <code>${VERIFICATION_CHANNEL_ID}</code></p>
    <form method="post" action="/admin/post-panel"><input type="hidden" name="channel_id" value="${VERIFICATION_CHANNEL_ID}">
    <button class="btn" type="submit">Post Panel to Verification Channel</button></form>
    <p class="muted" style="margin-top:12px">Or choose another channel:</p>
    <form method="post" action="/admin/post-panel" class="row"><select name="channel_id" required>
    <option value="">Choose Discord channel</option>${channelOpts}</select><button class="btn gray" type="submit">Post Elsewhere</button></form></div>
...
app.get('/auth/google/callback',async(req,res,next)=>{
  try{
    const st=await consumeState(req.query.state);
    if(!st||!['google_creator','google_creator_invite','google_creator_password'].includes(st.provider)) throw new Error('Google authorization state is invalid or expired.');
    let inviteToken=null;
    if(st.provider==='google_creator_invite'){
      inviteToken=st.context;
      const invite=(await db.query(`SELECT * FROM creator_invites WHERE token=$1`,[inviteToken])).rows[0];
      if(!invite||invite.used_at||new Date(invite.expires_at).getTime()<=Date.now()) throw new Error('This creator invite is invalid, expired, or already used.');
    }
    ...
    if(st.provider==='google_creator_invite'){
      await db.query(
        `UPDATE creator_invites SET used_at=NOW(),used_channel_id=$2 WHERE token=$1`,
        [st.context,ch.id]
      );
      return res.send(page('Creator Connected',`
      <div class="card">
        <h1>Creator Connected Successfully</h1>
        <p><strong>${esc(ch.name)}</strong> is now connected.</p>
        <p>You can close this page and tell the server owner you're done.</p>
      </div>`));
    }

    if(st.provider==='google_creator_password'){
      return res.send(page('Creator Connected',`
      <div class="card">
        <h1>Creator Connected Successfully</h1>
        <p><strong>${esc(ch.name)}</strong> is now connected to Safetybot.</p>
...
app.post('/admin/creator-invite',adminOnly,async(req,res,next)=>{
  try{
    const hours=Math.max(1,Math.min(168,Number(req.body.hours||24)));
    const token=crypto.randomBytes(24).toString('hex');
    const expires=new Date(Date.now()+hours*60*60*1000);
    await db.query(
      `INSERT INTO creator_invites(token,expires_at) VALUES($1,$2)`,
      [token,expires]
    );
    const link=`${config.baseUrl}/creator/invite/${token}`;
    res.send(page('Creator Invite Created',`
    <div class="card"><h1>Creator Invite Created</h1>
    <p>Send this single-use link to your creator:</p>
    <p><input style="width:95%" value="${esc(link)}" readonly onclick="this.select()"></p>
    <p class="muted">Expires: ${esc(expires.toLocaleString())}</p>
    <a class="btn" href="/admin">Back to Admin</a></div>`));
  }catch(e){next(e);}
});

app.get('/creator/invite/:token',async(req,res,next)=>{
  try{
    const row=(await db.query(
      `SELECT * FROM creator_invites WHERE token=$1`,
      [req.params.token]
    )).rows[0];

    if(!row) return res.status(404).send(page('Invite Not Found','<div class="card"><h1>Invalid invite link</h1></div>'));
    if(row.used_at) return res.status(410).send(page('Invite Used','<div class="card"><h1>This invite link has already been used.</h1></div>'));
    if(new Date(row.expires_at).getTime()<=Date.now()) return res.status(410).send(page('Invite Expired','<div class="card"><h1>This invite link has expired.</h1></div>'));

    const state=await createState('google_creator_invite',req.params.token);
    const auth=makeGoogleClient();
    res.redirect(auth.generateAuthUrl({
      access_type:'offline',prompt:'consent',state,
      scope:[config.google.membershipScope,config.google.readonlyScope]
